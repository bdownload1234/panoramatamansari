...
class Marketing extends CI_Controller {
   ...
   var $data_ref = array('uri_controllers' => 'marketing');
		...
		foreach ($list as $post) {
			...
			if($post->foto == ''){
				if($post->jenis_kelamin == 'Laki-laki'){
					$row[] = '<img class="direct-chat-img" src="'.base_url('/assets/images/pria.jpg').'"><br>'.$post->kode_marketing;
				}else if($post->jenis_kelamin == 'Perempuan'){
					$row[] = '<img class="direct-chat-img" src="'.base_url('/assets/images/wanita.jpg').'"><br>'.$post->kode_marketing;
				}
			}else{
				$row[] = '<img class="direct-chat-img" src="'.base_url('berkas_user/'.$post->foto).'"><br>'.$post->kode_marketing;
			}
	...
	private function _do_upload(){
		...
		$config['upload_path']          = './berkas_user/';
