...
class Kavling extends CI_Controller {
   ...
   var $data_ref = array('uri_controllers' => 'kavling');
	...
	{
		$data['downData'] = $this->db->query("SELECT * FROM kavling_peta k 
    		LEFT JOIN customer c ON c.id_kavling = k.id_kavling 
        		ORDER BY 
            SUBSTRING(kode_kavling, 1, 1),
            CAST(SUBSTRING(kode_kavling, 2, 1) AS UNSIGNED),
            CAST(SUBSTRING(kode_kavling, 4) AS UNSIGNED)")->result();
	...
	public function ajax_edit($id)
	{
		...
		$data['harga_jual_ajb'] = rupiah($data['harga_jual_ajb']);
	...
	public function ajax_update()
	{
		...
		// }else{
			$data = array(
				...
				'stt_kavling' 			=> $this->input->post('stt_kavling'),
				'keterangan' 			=> $this->input->post('keterangan'),
				'model_rumah' 			=> $this->input->post('model_rumah'),
		...
		$this->db->insert('log_hrg_kavling', [
		    'id_kavling' => $this->input->post('id'),
		    'hrg_jual' => str_replace('.','', $this->input->post('harga_jual')),
		    'harga_jual_ajb' => str_replace('.','', $this->input->post('harga_jual_ajb')),
		    'created_at' => date("Y-m-d H:i:s"),
		    'created_by' => $query_user->username,
	    ]);
