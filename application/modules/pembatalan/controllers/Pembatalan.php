...
// 		var_dump($list);
		...
		foreach ($list as $post) {
			...
			$link_form = ' <a class="btn btn-xs btn-success" target="_blank" href="'.base_url('pembatalan/cetak_form/'.$post->id_pembatalan).'">Cetak Form</a> ';
	...
	public function ajax_add()
	{
		...
		$cek_log_kavling = $this->db->query("SELECT * FROM log_hrg_kavling WHERE id_kavling = '".$this->input->post('id_kavling')."' ORDER BY id DESC LIMIT 1 ")->row_array();
		$this->db->update('kavling_peta', [
    		    'stt_kavling' => '1',
    	    ], ['id_kavling' => $this->input->post('id_kavling')]);
		if($cek_log_kavling <> NULL){
    		$this->db->update('kavling_peta', [
    		    'hrg_jual' => $cek_log_kavling['hrg_jual'],
    		    'harga_jual_ajb' => $cek_log_kavling['harga_jual_ajb'],
    	    ], ['id_kavling' => $this->input->post('id_kavling')]);
		}
    ...
    function cetak_form($idBatal){
        // panggil library yang kita buat sebelumnya yang bernama pdfgenerator
        $this->load->library('pdfgenerator');
        
        // title dari pdf
        $this->db->select('pembatalan.*, spr.nomor_spr, spr.tanggal_spr, customer.nama_lengkap, kavling_peta.kode_kavling, kavling_peta.tipe_bangunan, kavling_peta.model_rumah');
		$this->db->join('customer', 'customer.id_customer = pembatalan.id_customer', 'left');
		$this->db->join('kavling_peta', 'kavling_peta.id_kavling = pembatalan.id_kavling', 'left');
		$this->db->join('spr', 'spr.id_customer = customer.id_customer', 'left');
        $this->db->from('pembatalan');
        $this->db->where('pembatalan.id_pembatalan', $idBatal);
        $this->data['data'] = $this->db->get()->row_array();
        $this->data['title_pdf'] = 'Form Pembatalan';
    ...
    {
        $this->db->from('pembatalan');
		$this->db->select('pembatalan.*, customer.nama_lengkap, customer.no_telp, kavling_peta.kode_kavling');
		$this->db->join('customer', 'pembatalan.id_customer = customer.id_customer', 'left');
		$this->db->join('kavling_peta', 'kavling_peta.id_kavling = pembatalan.id_kavling', 'left');
