   ...
   var $data_ref = array('uri_controllers' => 'pengguna');
	...
	public function ajax_list()
	{
		...
		foreach ($list as $post) {
			...
			if($post->kirim_notif == '1'){
				$notif = '<span class="badge badge-success">Ya</span>';
			}else{
				$notif = '<span class="badge badge-danger">Tidak</span>';
			}
			...
			$row = array();
         	...
         	$row[] = $post->surname;
			$row[] = $post->username;
         	$row[] = $post->no_wa.' '.$notif;
         	$row[] = $post->email;
			if($post->is_admin == '1'){
				$row[] = '<span class="badge badge-primary">Admin Panorama</span>';
			}else if($post->is_admin == '0'){
				$row[] = '<span class="badge badge-warning">Direktur Utama</span>';
			}else if($post->is_admin == '2'){
				$row[] = '<span class="badge badge-info">Admin KPR</span>';
			}else if($post->is_admin == '3'){
				$row[] = '<span class="badge badge-secondary">PIC Pusat</span>';
			}else if($post->is_admin == '4'){
				$row[] = '<span class="badge badge-info">PIC Lapangan</span>';
			}else if($post->is_admin == '5'){
				$row[] = '<span class="badge badge-info">Admin Keuangan</span>';
			}else if($post->is_admin == '6'){
				$row[] = '<span class="badge badge-info">Admin Pajak</span>';
			}else{
				$row[] = '<span class="badge badge-danger">ILEGAL</span>';
			}
	...
	public function ajax_add()
	{
      ...
      // $user = $this->ion_auth->user()->row();
		$data = array(
				...
				'kirim_notif' 	=> $this->input->post('kirim_notif'),
