...
class Dashboard_pembangunan extends CI_Controller {
	...
	{
		$data = $this->db->query("SELECT a.*, b.*, c.status_kavling as status_pembangunan FROM kavling_peta a 
			LEFT JOIN customer b ON a.nik_customer = b.nik  
			LEFT JOIN kavling_pembangunan c ON a.id_kavling = c.id_kavling
	...
	{
	    $id_kavling = $this->input->post('id');
	    $kode_kavling = $this->input->post('kode');
	    $status_kavling = $this->input->post('status');
	   // $spk = $this->input->post('kode');
	   
	   $check = $this->db->query("SELECT COUNT(*) AS count FROM kavling_pembangunan WHERE id_kavling = ".$id_kavling)->row();
	   if($check->count > 0){
	       $this->db->update('kavling_pembangunan', ['status_kavling' => $status_kavling], ['id_kavling' => $id_kavling]);
	   }else{
	       $this->db->insert('kavling_pembangunan', [
	           'id_kavling' => $id_kavling,
	           'kode_kavling' => $kode_kavling,
	           'status_kavling' => $status_kavling,
           ]);
	   }
