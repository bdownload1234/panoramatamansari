...
class Marketing extends CI_Controller
{
    ...
    public function ajax_list()
    {
        ...
        foreach ($list as $post) {
            ...
            $row[] = $post->no_telp_marketing;
            $row[] = $post->alamat_marketing;
            // $row[] = "";
            ...
            $row[] = $post->nik;
            $row[] = is_null($post->ktp) ? '' : '<img class="direct-chat-img" src="' . base_url('foto_marketing/ktp/' . $post->ktp) . '">';
            $row[] = $post->npwp;
            $row[] = is_null($post->doc_npwp) ? '' : '<img class="direct-chat-img" src="' . base_url('foto_marketing/npwp/' . $post->doc_npwp) . '">';
            $row[] = $post->persentase_fee_marketing;
            $row[] = rupiah($post->harga_jual_ajb);;
    ...
    public function ajax_add()
    {
        $data = [
            ...
            'nama_agen' => strtoupper($this->input->post('nama_agen')),
            'jenis_kelamin' => $this->input->post('jenis_kelamin'),
            'alamat_marketing' => $this->input->post('alamat'),
            'no_telp_marketing' => $this->input->post('no_telp'),
            'nik' => $this->input->post('nik'),
            'npwp' => $this->input->post('npwp'),
            'persentase_fee_marketing' => $this->input->post('persentase_fee_marketing'),
   ...
   <td>' .
    ...
    {
        $config['upload_path'] = './foto_marketing/ktp/';
        $config['allowed_types'] = 'gif|jpg|png|pdf';
        $config['max_size'] = 1000; //set max size allowed in Kilobyte
        $config['max_width'] = 3000; // set max width image allowed
        $config['max_height'] = 3000; // set max height allowed
    ...
    {
        $config['upload_path'] = './foto_marketing/npwp/';
        $config['allowed_types'] = 'gif|jpg|png|pdf';
        $config['max_size'] = 1000; //set max size allowed in Kilobyte
        $config['max_width'] = 3000; // set max width image allowed
        $config['max_height'] = 3000; // set max height allowed
