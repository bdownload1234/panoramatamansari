...
class Profile extends CI_Controller {
   ...
   var $data_ref = array('uri_controllers' => 'profile');
...
// 		$user_data['reg'] = $this->db->get_where('customer', ['email' => $this->encryption->decrypt($this->session->userdata('email'))])->row_array();

        $this->db->select('customer.*, kavling_peta.tipe_bangunan, kavling_peta.model_rumah');
		$this->db->from('customer');
		$this->db->join('kavling_peta', 'customer.lokasi_kavling = kavling_peta.kode_kavling', 'left');
		$this->db->where('email',$this->encryption->decrypt($this->session->userdata('email')));
		$query = $this->db->get();
		$user_data['reg'] = $query->row_array();
			
		$this->load->view('template_cust/header',$user_data);
	...
	public function password()
	{
		...
		$this->load->view('template_cust/header',$user_data);
		$this->load->view('password',$user_data);
      // $this->load->view('template/footer',$user_data);
	...
	public function gantipassword()
	{
		...
		$idCustomer = $cust['id_customer'];
		$param = [
			'password' 		=> password_hash($this->input->post('password'), PASSWORD_DEFAULT)
		];
		$this->db->update('customer', $param, ['id_customer' => $idCustomer]);
			
		redirect('profile/changesuccess');
	...
	public function changesuccess()
	{
		...
		$this->load->view('template_cust/header');
		$this->load->view('changesuccess');
	}
