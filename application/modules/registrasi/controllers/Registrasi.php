...
class Registrasi extends CI_Controller {
   ...
   var $data_ref = array('uri_controllers' => 'registrasi');
	...
	public function proses_registrasi()
	{
		...
		if($this->input->post('verifikasi') == '1'){
			$paramCust = [
				...
				'status_customer' 					=> ($this->input->post('verifikasi') == '1' ? '3' : '6'), 
				'nup_spr' 					        => $this->input->post('nup_spr'), 
			...
			if($cek){
				// Jika data sudah ada di tabel customer tinggal update saja
				$this->db->update('customer', $paramCust, ['id_registrasi' => $idREG]);
			}else{
				// jika belum ada, masukkan ke dalam data Customer
				$this->db->insert('customer', $paramCust);
				$idCust = $this->db->insert_id();
				
				if($this->input->post('nup_spr') == '1'){
    				// Buat SPR sesuai data registrasi
    				$this->buat_spr($idCust);
				}else{
				    $tahun = date('Y');
            		$nomor = $this->db->query("SELECT MAX(no_nup) as besar FROM nup WHERE no_nup like '%$tahun'")->row_array();
            		$noSekarangTRX = $nomor['besar'];
            		$urutanTRX = (int) substr($noSekarangTRX, 0, 5);
            		$urutanTRX++;
            		$hurufTRX = "/NUP-PTS/$tahun";
            		$noTraSekarang = sprintf("%05s", $urutanTRX).$hurufTRX;
				    $paramNup = [
				        'no_nup' => $noTraSekarang,
				        'id_customer' => $this->input->post('id_registrasi'),
				        'id_kavling' => $this->input->post('lokasi_kavling_id'),
				        'status' => 1,
				        'created_at' => date("Y-m-d H:i:s"),
			        ];
				    $this->db->insert('nup', $paramNup);
				}
			}
			...
			// masukkan kedala tabel keuangan booking
			if($this->input->post('status_kavling') == '3'){ // JIKA BOOKING
				$param = array(
					'tanggal' 			=> date('Y-m-d'), 
					'id_registrasi' 	=> $idREG, 
					'nominal' 			=> str_replace('.', '', $this->input->post('booking_fee'))
				);
				$this->db->insert('keuangan_booking', $param);
			}else if($this->input->post('status_kavling') == '2'){ // JIKA NUP
				$param = array(
					'tanggal' 			=> date('Y-m-d'), 
					'id_registrasi' 	=> $idREG, 
					'nominal' 			=> str_replace('.', '', $this->input->post('booking_fee'))
				);
				$this->db->insert('keuangan_nup', $param);
			}

			// Masukkan data fee marketing
			$param = array(
				'tanggal_transaksi' 	=> date('Y-m-d'), 
				'id_registrasi' 		=> $idREG, 
				'id_kavling'			=> $this->input->post('lokasi_kavling_id'),
				'id_marketing' 			=> $this->input->post('id_marketing'),
				'nominal_fee' 			=> str_replace('.', '', $this->input->post('fee_marketing')),
				'stt_fee'				=> '0'		
			);
			$this->db->insert('keuangan_fee_marketing', $param);
			...
			// Simpan tanpa membuat SPR (JIka data Customer sudah ada)
			$paramCust = [
				'id_registrasi' 				=> $this->input->post('id_registrasi'),
				'nama_lengkap' 					=> $this->input->post('nama_lengkap'),
				'nik' 							=> $this->input->post('nik'),
	...
	public function verifikasi($id)
	{
		...
		->select('registrasi.*, kavling_peta.*, marketing.nama_marketing')
	...
	public function buat_spr($idCustomer ='')
	{
		...
		$param = array(
			...
			'tanggal_spr' 			=> $tanggal,
			'jam_spr' 				=> $jam,
			'harga_rumah' 			=> is_null($rumah['harga_jual_ajb']) ? 0 : $rumah['harga_jual_ajb'],
			'harga_diskon' 			=> is_null($rumah['harga_diskon']) ? 0 : $rumah['harga_diskon'],
