           ...
           <table id="table" class="table table-striped table-hover table-bordered" cellspacing="0" width="100%">
                <thead>
                    <tr>
                        ...
                        <td align="center" width="15%">Nomor Kavling</td>
                        <td align="center" width="15%">Jumlah Kavling</td>
                        <td align="center" width="15%">Selesai legalitas</td>
                        <td align="center" width="20%">Keterangan</td>
                        <td align="center" width="20%">Detail Legalitas</td>
                    </tr>
                </thead>
                <tbody>
                    <?php 
                    $no=1;
                    $legalitas = $this->db->query("SELECT *, blok as b, (SELECT COUNT(*) as jum FROM kavling_peta s WHERE s.blok=b) as jumlah, 
                    (SELECT MIN(kode_kavling) FROM kavling_peta s WHERE s.blok=b) as mulai,   
                    (SELECT MAX(kode_kavling) FROM kavling_peta s WHERE s.blok=b) as sampai  
                    FROM kavling_peta GROUP BY blok ASC")->result();

                    foreach($legalitas as $lg){

                        // jumlah legalitas yang sudah selesai
                        $legalSelesai = $this->db->query("SELECT COUNT(*) as jum FROM legal WHERE blok = '$lg->blok'")->row_array();

                        echo '<tr>
                        <td align="center">'.$no++.'</td>
                        <td align="center">'.$lg->blok.'</td>
                        <td align="center">'.$lg->mulai.' sampai '.$lg->sampai.'</td>
                        <td align="center">'.$lg->jumlah.'</td>
                        <td align="center">'.$legalSelesai['jum'].'</td>
                        <td align="center"></td>
                        <td align="center"><a href="'.base_url('legal/detailblok/'.$lg->blok).'" class="btn btn-info btn-sm">detail</a></td>
                    </tr>';
                    }
                    ?>
